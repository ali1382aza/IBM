...
async def option_two_command(message: types.Message):
    ...
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(types.KeyboardButton("Option One22"), types.KeyboardButton("Option Two33"), types.KeyboardButton("Option Three56"))
    keyboard.add(types.KeyboardButton("Back"))  # Adding "Back" button
    await message.reply("Please choose an option:", reply_markup=keyboard)
...
async def main():bot = Bot(token="6819049569:AAHdaEjILbHFL7EvMBXh64Io1FAFfLrKC2U", proxy="http://10.111.222.1:8080/")
